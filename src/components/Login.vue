    ...
    methods: {
        ...
        login(){
            this.$refs.loginFromRef.validate( async valid =>{
                // console.log(valid);
                if(!valid) return;
                //优化  async   前面加 await   await只能修饰在async中
                ...
                //1.1项目中出现了登录之外的其他API接口，必须在登陆之后访问
                //1.2token只应在当前网站打卡期间生效，所以将token保存在sessionStroage中
               
                // console.log("登录成功后"+rel.data);
                window.sessionStorage.setItem("token",res.data.token);
                //  //2通过编程时导航跳转到后台主页，路由地址是/home
